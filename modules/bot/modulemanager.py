...
class ModuleManager(Module):
    ...
    async def moduleblack(self, message, content):
        ...
        # Need to update entry in db as well as the flag maintained in each module
        strings = list()
        self.db['module blacklist'] = blacklist.update(modules_to_blacklist)
        self.__save_db()
        for m in self.available_modules:
            if m.full_name in modules_to_blacklist:
                m.is_blacklisted = True
                strings.append(m.full_name)

        strings = ['Module(s)'] + strings + ['were blacklisted']
    ...
    async def modulewhite(self, message, content):
        ...
        strings = ['Module(s)'] + strings + ['were whitelisted']
        for msg in self.pack_into_messages(strings, delimiter=' '):
            await self.client.send_message(message.channel, msg)
