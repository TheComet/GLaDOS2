 ...
 */
 ...
 /* Modified by slicer4ever for python support. */
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <vector>
...
static PyObject *apng2gif_convert(PyObject *Self, PyObject *Args){
	char *InputFilename;
	char *OutputFilename;
	char szOut[256];
	...
	unsigned int xSize;
	unsigned int ySize;
	unsigned int TexelSize = 4; //seems this library enforces png's to 4 byte sized texels.
	unsigned int Flip = 0;
	tlevel = 128;
	bcolor = -1;
	back_r = back_g = back_b = 0;
	if(!PyArg_ParseTuple(Args, "ssiiiii", &InputFilename, &OutputFilename, &xOffset, &yOffset, &xSize, &ySize, &Flip)) return NULL;
	strncpy(szOut, OutputFilename, sizeof(szOut));
	std::vector<APNGFrame> frames;
	int res = load_apng(InputFilename, frames, &num_loops);
	if(res<0) return NULL;
	unsigned int NbrFrames = frames.size();
	// zero means first frame is hidden, so let's remove it (unless it's the only frame)
	if(NbrFrames==1 || (!res && frames.size()>1)){
		delete[] frames[0].rows;
		delete[] frames[0].p;
		frames.erase(frames.begin());
		if(NbrFrames==1) Py_BuildValue("i", NbrFrames);
	}
	...
	if (!frames.empty()){
		...
		//Crop our image if necessary!
		if(xOffset>=0 && xOffset+xSize<=w && yOffset>=0 && yOffset+ySize<=h){
			for(size_t n=0;n<frames.size();n++){
				unsigned char *p = new unsigned char[xSize*ySize*TexelSize];
				png_bytep *rows = new png_bytep[ySize*sizeof(png_bytep)];
				if(Flip){
					for(unsigned int y=0;y<ySize;y++){
						rows[y] = (png_bytep)p+y*xSize*TexelSize;
						for(unsigned int x = 0;x<xSize;x++){
							rows[y][x*4+0] = frames[n].rows[y+yOffset][xOffset+(xSize-(x+1))*4+0];
							rows[y][x*4+1] = frames[n].rows[y+yOffset][xOffset+(xSize-(x+1))*4+1];
							rows[y][x*4+2] = frames[n].rows[y+yOffset][xOffset+(xSize-(x+1))*4+2];
							rows[y][x*4+3] = frames[n].rows[y+yOffset][xOffset+(xSize-(x+1))*4+3];
						}
					}
				}else{
					for(unsigned int y = 0;y<ySize;y++){
		...
		if (save_agif(OutputFilename, frames, pAGIF, num_loops) != 0){
		  printf("save_agif() failed: '%s'\n", szOut);
		  return NULL;
		}
	...
	return Py_BuildValue("i", NbrFrames);
}

static PyMethodDef apng2gifMethods[] = {
	{"convert", apng2gif_convert, METH_VARARGS, "Dumps an animated png into a series of frames with the specefied file name sequence."},
	{NULL, NULL, 0, NULL}
};
...
PyMODINIT_FUNC
initapngdis(void){
	(void)Py_InitModule("apng2gif", apng2gifMethods);
	
}*/

/* Version 3.0+ python entry. */
static struct PyModuleDef apng2gifModule = {
...
PyMODINIT_FUNC
PyInit_apng2gif(void){
	return PyModule_Create(&apng2gifModule);
}
