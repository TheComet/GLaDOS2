...
class Emotes(glados.Module):
    ...
    def request_pony_emote(self, message, content):
        ...
        yield from self.client.send_file(message.channel, emote_file)

    @glados.Module.commands('help')
    def request_list_of_emotes(self, message, content):
        if content == '':
            return tuple()

        emotes = [f.strip('.png').strip('.gif') for f in self.get_emotes_list()]
        yield from self.client.send_message(message.channel, 'I am sending you a list of pony emotes!')
